...
import com.uiptv.application.WatchingNowSeriesActionRequest;
...
public class HttpWatchingNowSeriesActionServer implements HttpHandler {
    ...
    private void upsert(HttpExchange ex) throws IOException {
        ...
        WatchingNowApplicationService.getInstance().saveSeriesEpisode(
                new WatchingNowSeriesActionRequest(
                        accountId,
                        categoryId,
                        seriesId,
                        episodeId,
    ...
    private void remove(HttpExchange ex) throws IOException {
        ...
        WatchingNowApplicationService.getInstance().removeSeries(accountId, categoryId, seriesId);
        writeJson(ex, 200, "{\"status\":\"ok\"}");
    }
