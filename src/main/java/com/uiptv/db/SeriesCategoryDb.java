...
public class SeriesCategoryDb extends BaseDb {
    private static SeriesCategoryDb instance;

    public SeriesCategoryDb() {
        super(SERIES_CATEGORY_TABLE);
    }

    public static synchronized SeriesCategoryDb get() {
        if (instance == null) {
            instance = new SeriesCategoryDb();
        }
        return instance;
    }
    ...
    public boolean isFresh(Account account, long maxAgeMs) {
        String sql = "SELECT MAX(cachedAt) FROM " + SERIES_CATEGORY_TABLE.getTableName() + " WHERE accountId=? AND accountType=?";
    ...
    public void deleteByAccount(String accountId) {
        String sql = "DELETE FROM " + SERIES_CATEGORY_TABLE.getTableName() + " WHERE accountId=?";
        try (Connection conn = connect(); PreparedStatement statement = conn.prepareStatement(sql)) {
    ...
    private void insert(Category category, Account account, long cachedAt) {
        try (Connection conn = connect(); PreparedStatement statement = conn.prepareStatement(insertTableSql(SERIES_CATEGORY_TABLE))) {
