...
public class VodChannelDb extends BaseDb {
    ...
    public static synchronized VodChannelDb get() {
        if (instance == null) {
            instance = new VodChannelDb();
        }
        return instance;
    }
    ...
    public Channel getChannelByChannelId(String channelId, String categoryId, String accountId) {
        List<Channel> channels = getAll(
                ...
                new String[]{channelId, categoryId, accountId}
        );
    ...
    public void deleteByAccount(String accountId) {
        String sql = "DELETE FROM " + VOD_CHANNEL_TABLE.getTableName() + " WHERE accountId=?";
        try (Connection conn = connect(); PreparedStatement statement = conn.prepareStatement(sql)) {
    ...
    private void insert(Channel channel, String categoryId, Account account, long cachedAt) {
        try (Connection conn = connect(); PreparedStatement statement = conn.prepareStatement(insertTableSql(VOD_CHANNEL_TABLE))) {
