...
class HttpPlayerJsonServer : HttpHandler {
    ...
    private fun resolvePlayback(ex: HttpExchange, mode: String): ResolvedWebPlayback {
        ...
        val accountId = getParam(ex, "accountId")
        val bingeWatchToken = getParam(ex, "bingeWatchToken")
        val bingeWatchEpisodeId = getParam(ex, "episodeId")
        val categoryId = getParam(ex, "categoryId")
        val channelId = getParam(ex, "channelId")
        val directUrl = getParam(ex, "url")
        val seriesParentId = getParam(ex, "seriesParentId")
    ...
    private fun resolveRequestedMode(ex: HttpExchange?, requestedMode: String?): String {
        ...
        return when {
            PATH_PLAYER_BINGEWATCH == path || PATH_PLAYER_SERIES == path -> MODE_SERIES
            PATH_PLAYER_VOD == path -> MODE_VOD
            PATH_PLAYER_LIVE == path -> MODE_ITV
            else -> ""
        }
    }
    ...
    private fun resolveBingeWatchPlayback(ex: HttpExchange, accountId: String?, token: String, episodeId: String?): ResolvedWebPlayback {
    ...
    private fun applyWebPlaybackProcessing(response: PlayerResponse?, mode: String, hvec: String?, preferHls: Boolean) {
        ...
        val allowTranscoding = ConfigurationService.getInstance().read().enableFfmpegTranscoding
    ...
    private fun shouldStartTransmuxing(response: PlayerResponse, mode: String, originalUrl: String, preferHls: Boolean): Boolean {
        ...
        return forceWebHls || FfmpegService.getInstance().isTransmuxingNeeded(response.url ?: "")
    }
    ...
    private fun applyTransmuxedPlayback(response: PlayerResponse, mode: String, originalUrl: String, hvec: String?, preferHls: Boolean, allowTranscoding: Boolean) {
        ...
        val vodStylePlaylist = shouldUseVodStylePlaylist()
    ...
    private fun shouldUseVodStylePlaylist(): Boolean = WEB_VOD_STYLE_PLAYLIST
    ...
    private fun tryStartTransmuxingInput(inputUrl: String, forceWebHls: Boolean): Boolean =
        try {
            FfmpegService.getInstance().startTransmuxing(inputUrl, forceWebHls)
        } catch (_: Exception) {
            false
        }
    ...
    private fun tryStartTranscodingInput(inputUrl: String, forceWebHls: Boolean): Boolean =
        try {
            FfmpegService.getInstance().startTranscoding(inputUrl, forceWebHls)
        } catch (_: Exception) {
            false
        }
    ...
    private fun resolveBookmarkPlayback(bookmarkId: String, mode: String, seriesParentId: String?): PlayerResponse =
        playerRequestResolver.resolveBookmarkPlayback(bookmarkId, mode, seriesParentId)

    private fun resolveDirectPlayback(ex: HttpExchange, accountId: String?, categoryId: String?, channelId: String?, mode: String, seriesParentId: String?): PlayerResponse {
        val account = AccountService.getInstance().getById(accountId)
        val seriesId = getParam(ex, "seriesId")
        val requestChannel = buildRequestChannel(channelId, ex)
    ...
    private fun buildRequestChannel(channelId: String?, ex: HttpExchange): Channel =
        Channel().apply {
            ...
            name = sanitizeParam(getParam(ex, "name"))
            logo = sanitizeParam(getParam(ex, "logo"))
            cmd = sanitizeParam(getParam(ex, "cmd"))
            cmd_1 = sanitizeParam(getParam(ex, "cmd_1"))
            cmd_2 = sanitizeParam(getParam(ex, "cmd_2"))
            cmd_3 = sanitizeParam(getParam(ex, "cmd_3"))
            drmType = sanitizeParam(getParam(ex, "drmType"))
            drmLicenseUrl = sanitizeParam(getParam(ex, DRM_LICENSE_URL))
            clearKeysJson = sanitizeParam(getParam(ex, CLEAR_KEYS_JSON))
            inputstreamaddon = sanitizeParam(getParam(ex, INPUTSTREAMADDON))
    ...
    private fun appendChannelJson(json: JSONObject, response: PlayerResponse?) {
        ...
        if (channel == null || !hasChannelMetadata(channel)) return
    ...
    private fun appendDrmJson(json: JSONObject, response: PlayerResponse?) {
        ...
        val clearKeys = parseClearKeysJson(response.clearKeysJson)
        if (clearKeys != null && !clearKeys.isEmpty) drm.put("clearKeys", clearKeys)
    ...
    private fun buildBingeWatchItems(items: List<BingeWatchService.PlaylistItem>): JSONArray {
    ...
    private fun safe(value: String?): String {
        ...
        if ("null".equals(normalized, true) || "undefined".equals(normalized, true)) return ""
        return normalized
    }
    ...
    private fun shouldForceWebHls(mode: String, response: PlayerResponse?): Boolean {
        ...
        if (MODE_SERIES != normalizedMode) return false
        return shouldForceSeriesWebHls((response.url ?: "").lowercase())
    }
    ...
    private fun shouldForceWebHlsForUrl(mode: String, url: String): Boolean {
        ...
        val normalizedMode = if (isBlank(mode)) "" else mode.trim().lowercase()
        if (MODE_SERIES != normalizedMode) return false
        return shouldForceSeriesWebHls(url.lowercase())
    }
    ...
    private fun isClientDisconnect(throwable: Throwable?): Boolean {
        ...
        while (current != null) {
            if (current is IOException) {
                val message = sanitizeParam(current.message).lowercase()
                if (message.contains("broken pipe") || message.contains("connection reset") || message.contains("stream closed")) {
                    return true
                }
            }
    ...
    private fun isDigitsOnly(value: String?): Boolean = !isBlank(value) && value!!.all(Char::isDigit)
